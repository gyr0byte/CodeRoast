...
def generate_instagram_story_card(grade: str, score: float, language: str, roast: str) -> bytes:
    """Generate a high-res vertical 1080x1920 Instagram Story shareable card."""
    width, height = 1080, 1920
    img = Image.new("RGB", (width, height), color=(11, 13, 18))
    draw = ImageDraw.Draw(img)
    
    # Outer Neon Border
    draw.rectangle([(30, 30), (width - 30, height - 30)], outline=(168, 85, 247), width=6)
    draw.rectangle([(45, 45), (width - 45, height - 45)], outline=(236, 72, 153), width=2)
    
    font = ImageFont.load_default()
    
    # Top Header
    draw.text((80, 80), "OFFICIAL CODE VERDICT", fill=(148, 163, 184), font=font)
    draw.text((80, 120), "CODEROAST AI", fill=(168, 85, 247), font=font)
    draw.text((80, 160), "🎓 University Project Fair Edition", fill=(245, 158, 11), font=font)
    
    draw.line([(80, 210), (width - 80, 210)], fill=(51, 65, 85), width=3)
    
    # Grade & Score Box
    grade_clean = grade.encode("ascii", "ignore").decode("ascii").strip()
    grade_str = grade_clean.split(" ")[0] if grade_clean else "F"
    
    box_color = (239, 68, 68) if grade_str in ["F", "D"] else (16, 185, 129)
    draw.rectangle([(80, 250), (width - 80, 420)], outline=box_color, width=4)
    draw.text((120, 280), f"FINAL GRADE: {grade_str}", fill=box_color, font=font)
    draw.text((120, 330), f"OVERALL SCORE: {score}/100", fill=(255, 215, 0), font=font)
    draw.text((120, 370), f"LANGUAGE: {language.upper()}", fill=(203, 213, 225), font=font)
    
    # Roast Section Title
    draw.text((80, 470), "SENIOR DEV AI VERDICT:", fill=(168, 85, 247), font=font)
    ...
    for line in lines[:32]:
        draw.text((80, y), line, fill=(241, 245, 249), font=font)
        y += 35
        
    # Footer
    draw.line([(80, height - 120), (width - 80, height - 120)], fill=(51, 65, 85), width=2)
    draw.text((80, height - 90), "Generated by CodeRoast AI", fill=(148, 163, 184), font=font)
    draw.text((80, height - 60), "https://github.com/gyr0byte/CodeRoast", fill=(100, 116, 139), font=font)
...
with col2:
    ...
    if has_results and st.session_state.get("metrics"):
        ...
        # ─── Action Dock: Voice Narration & Shareable Cards ─────────────────────
        st.markdown("<br>", unsafe_allow_html=True)
        
        # 1. Voice Narration Audio Player
        st.markdown("##### 🔊 Voice Narration (Read Aloud)")
        audio_bytes = generate_roast_audio(clean_roast_text, lang_code="en" if target_lang == "english" else "hi")
        if audio_bytes:
            st.audio(audio_bytes, format="audio/mp3")
        else:
            st.caption("🔊 Voice narration generator offline.")

        # 2. Download Shareable Verdict Cards
        st.markdown("<br>##### 🎴 Download Shareable Verdict Cards", unsafe_allow_html=True)
        ...
        with btn_col1:
            st.download_button(
                label="🖼️ Landscape Card (PNG)",
                data=card_img_bytes,
                file_name=f"coderoast_{scores['grade'].split()[0]}_landscape.png",
                mime="image/png",
                use_container_width=True
            )
            
        story_card_bytes = generate_instagram_story_card(
            grade=scores["grade"],
            score=scores["overall"],
            language=language,
            roast=clean_roast_text
        ...
        with btn_col2:
            st.download_button(
                ...
                data=story_card_bytes,
                file_name=f"coderoast_{scores['grade'].split()[0]}_story.png",
                mime="image/png",
                use_container_width=True
            )
